...
module ForwardUnit #(
    ...
    // EX stage signals (NEW - producer between EXR and EX2)
    ...
    // CSR hazard signals
    input wire csr_hazard_ex2,          // NEW
    input wire csr_hazard_mem,
    input wire csr_hazard_wb,
    input wire [XLEN-1:0] EX2_csr_write_data,
    ...
    // ALU source forwarding selection (priority: EX > EX2 > MEM > WB > Retire)
    // 3'b000: no forwarding, 3'b001: EX, 3'b010: EX2, 3'b011: MEM, 3'b100: WB, 3'b101: Retire
    ...
    assign alu_forward_source_data_a =
            hazard_ex[0] ? EX_forward_data_value :
            hazard_ex2[0] ? EX2_forward_data_value :
    ...
    assign alu_forward_source_data_b =
            hazard_ex[1] ? EX_forward_data_value :
            hazard_ex2[1] ? EX2_forward_data_value :
            hazard_mem[1] ? MEM_forward_data_value :
            hazard_wb[1] ? WB_forward_data_value :
            hazard_retire[1] ? retire_forward_data_value : {XLEN{1'b0}};

    // Store data forwarding (EXR stage) - priority: EX > EX2 > MEM > WB > Retire
    assign store_forward_enable = store_hazard_ex || store_hazard_ex2 || store_hazard_mem ||
                                  store_hazard_wb || store_hazard_retire;
    assign store_forward_data = store_hazard_ex ? EX_forward_data_value :
                                store_hazard_ex2 ? EX2_forward_data_value :
                                store_hazard_mem ? MEM_forward_data_value :
    ...
    always @(*) begin
        // EX stage forward data selection.
        // The ALU has not produced a result yet in EX, so ALU and LOAD producers are
        // resolved by exr_data_stall in the Hazard Unit and never reach this mux.
        case (EX_opcode)
            `OPCODE_ENVIRONMENT : EX_forward_data_value = EX_csr_read_data;
            `OPCODE_LUI : EX_forward_data_value = EX_imm;
            `OPCODE_JAL : EX_forward_data_value = EX_pc_plus_4;
            `OPCODE_JALR : EX_forward_data_value = EX_pc_plus_4;
            default: EX_forward_data_value = {XLEN{1'b0}};
        endcase
        ...
        // CSR Forwarding
        if (csr_hazard_ex2) begin
            csr_forward_data_value = EX2_csr_write_data;
        end
        else if (csr_hazard_mem) begin
