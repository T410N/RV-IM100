...
module RV32IM72F7SP_CORE #(
    ...
    // dm_read_data / dm_write_done return path is assigned next to the
    // data_memory_read_data / write_done declarations below.
    ...
    // NOTE (Core-Only): im_instruction is now a module input port.
    ...
    // NOTE (Core-Only): ROM bypass data is resolved outside the core and
    // returned through dm_read_data, so rom_read_data no longer exists here.
    ...
    // Core-Only: return path from the external data memory
    ...
    // EX_EX2_Register
    wire EX_is_load = (EX_opcode == `OPCODE_LOAD);
    wire EX2_is_load;
    ...
    // NOTE (Core-Only): DataMemory instance removed; driven through the
    // dm_* port group above.
    ...
    // NOTE (Core-Only): InstructionMemory instance removed; driven through the
    // im_* port group above.
    ...
    // Pre-compute hazard signals in ID stage (avoids opcode decode in critical path)
    ...
    wire [2:0] store_forward_select;
    reg [XLEN-1:0] store_forward_data_muxed;
    always @(*) begin
        case (store_forward_select)
            3'b001: store_forward_data_muxed = EX2_forward_data;
            3'b010: store_forward_data_muxed = MEM_forward_data;
            3'b011: store_forward_data_muxed = WB_forward_data_value;
            3'b100: store_forward_data_muxed = retire_forward_data_value;
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            retire_forward_data_value <= {XLEN{1'b0}};
            instruction_retired <= 1'b0;
        end 
        else if (clk_enable) begin
            retire_rd <= WB_rd;
            retire_register_write_enable <= WB_register_write_enable;
            retire_forward_data_value <= WB_forward_data_value;
    ...
    always @(*) begin
        ...
        case (EX_alu_src_B_select)
            ...
            `ALU_SRC_B_SHAMT: alu_normal_source_b = {26'b0, EX_imm[4:0]};
        ...
        case (alu_forward_source_select_a)
            ...
            3'b100: src_A = retire_forward_data_value;
        ...
        case (alu_forward_source_select_b)
            ...
            3'b100: src_B = retire_forward_data_value;
    ...
    always @(*) begin
        case (EX_opcode)
            ...
            default: EX_forward_select = 3'd0;
        endcase
    end 
