...
module RV64IM72F6SP_CORE #(
    ...
    // IM interface (combinational - no stall signals needed)
    assign im_pc = pc;

    // DM interface (no write_done/read_stall - single-cycle access)
    ...
    assign IF_imm = {{(XLEN-13){instruction[31]}}, instruction[31], instruction[7], instruction[30:25], instruction[11:8], 1'b0};
