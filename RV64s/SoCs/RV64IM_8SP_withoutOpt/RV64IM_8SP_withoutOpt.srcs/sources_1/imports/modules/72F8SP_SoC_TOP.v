module RV64IM72F8SPSoCTOP #(
    ...
    // CPU clock from the clocking wizard: 96 MHz derived from the 100 MHz board
    // clock.  The 8-stage core without the RTL optimizations does not close
    // timing at 100 MHz, so the core runs on clk_cpu instead of the board clock.
    wire clk_cpu;
    wire clk_locked;
    wire reset = ~reset_n;
    ...
    RV64IM72F8SP #(.XLEN(XLEN)) rv64im72f_6sp (
        .clk(clk_cpu),
        .clk_enable(cpu_clk_enable),
        .reset(internal_reset),
        .UART_busy(tx_busy),
