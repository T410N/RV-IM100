module DataMemory #(
    ...
    // Byte-banked BRAM (4 banks × 8-bit × 16384 entries = 64KB)
    // Bank count halves with XLEN; depth doubles so total capacity matches the
    // RV64 build, keeping the BRAM resource comparison like-for-like.
    (* ram_style = "block" *) reg [7:0] memory_bank0 [0:16383];
    (* ram_style = "block" *) reg [7:0] memory_bank1 [0:16383];
    (* ram_style = "block" *) reg [7:0] memory_bank2 [0:16383];
    (* ram_style = "block" *) reg [7:0] memory_bank3 [0:16383];
    ...
    always @(posedge clk) begin
        if (clk_enable && !read_stall) begin
            ram_read_data <= {memory_bank3[ram_address],
